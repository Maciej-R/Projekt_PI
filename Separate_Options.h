...
vector<string> separate_options(char*, int, char = '-');

template<typename Pred>
vector<string> separate_options(char* opt, int end, Pred pred) {
	...
	for (; it < end; ++it) {		//Iterates through whole input without command name
		...
		if (pred(opt[it])) {		//Options begin with -
			...
			else result.push_back(string(opt + it, (l - it + 1)));					//Last value is second space
